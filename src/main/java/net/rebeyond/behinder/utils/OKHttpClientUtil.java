...
public class OKHttpClientUtil {
   ...
   public static Map<String, Object> post(String url, Map<String, String> headers, byte[] requestBody) throws IOException {
   ...
   static {
      spec = (new ConnectionSpec.Builder(ConnectionSpec.MODERN_TLS)).tlsVersions(TlsVersion.TLS_1_0, TlsVersion.TLS_1_1, TlsVersion.TLS_1_2, TlsVersion.TLS_1_3).build();
      client = (new OkHttpClient()).newBuilder().connectTimeout(10L, TimeUnit.SECONDS).sslSocketFactory(SSLSocketClient.getSSLSocketFactory(), SSLSocketClient.getX509TrustManager()).hostnameVerifier(SSLSocketClient.getHostnameVerifier()).connectionSpecs(Arrays.asList(ConnectionSpec.MODERN_TLS, ConnectionSpec.COMPATIBLE_TLS, ConnectionSpec.CLEARTEXT, spec)).readTimeout(60L, TimeUnit.SECONDS).proxy((Proxy)MainController.currentProxy.get("proxy")).connectionPool(new ConnectionPool(100, 30L, TimeUnit.SECONDS)).cookieJar(new CookieJar() {
         public void saveFromResponse(HttpUrl url, List<Cookie> cookiesx) {
            Set<Cookie> cookieSet = new HashSet(cookiesx);
            List<Cookie> cookies = new ArrayList(cookieSet);
            OKHttpClientUtil.cookieStore.put(url.host(), cookies);
         }

         public List<Cookie> loadForRequest(HttpUrl url) {
            List<Cookie> cookies = (List)OKHttpClientUtil.cookieStore.get(url.host());
      ...
      private Response unzip(Response response) throws IOException {
         if (response.body() == null) {
            ...
            if (contentEncoding != null && contentEncoding.equals("gzip")) {
               ...
               return response.newBuilder().headers(strippedHeaders).body(new RealResponseBody(response.body().contentType().toString(), -1L, Okio.buffer((Source)responseBody))).build();
            } else {
               return response;
            }
         }
