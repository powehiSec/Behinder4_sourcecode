...
public class MainController {
   ...
   private int COL_INDEX_URL = 1;
   private int COL_INDEX_IP = 2;
   private int COL_INDEX_TYPE = 3;
   private int COL_INDEX_OS = 4;
   private int COL_INDEX_COMMENT = 5;
   private int COL_INDEX_ADDTIME = 6;
   private int COL_INDEX_STATUS = 7;
   private int COL_INDEX_ID = 0;
   ...
   private void initIcons() {
      try {
         byte[] resourceData = Utils.getResourceData("net/rebeyond/behinder/resource/search.png");
         this.searchShellLabel.setGraphic(new ImageView(new Image(new ByteArrayInputStream(resourceData))));
      } catch (Exception var2) {
      }
   ...
   private void injectAgentNoFile(ShellService shellService, String path, boolean isAntiAgent) throws Exception {
      ...
      if (responseObj.getString("status").equals("success")) {
         ...
         cp.insertClassPath((ClassPath)(new ByteArrayClassPath(className, targetClassByte)));
   ...
   private void initShellTable() throws Exception {
      shellListTable.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
      ObservableList<TableColumn<List<StringProperty>, ?>> tcs = this.shellListTable.getColumns();

      for (int i = 0; i < tcs.size(); i++) {
         int finalI = i;
         (tcs.get(i)).setCellValueFactory((data) -> {
            return ((ObservableValue) ((List) ((TableColumn.CellDataFeatures) data).getValue()).get(finalI));
         });
      }
               ...
               if (empty) {
                  ...
                  if (rowItem == null) {
                     ...
                     try {
                        ...
                        String memType = ((StringProperty)((List)this.getTableRow().getItem()).get(MainController.this.COL_INDEX_MEMTYPE-1)).getValue();
   ...
   private void loadContextMenu() {
      ...
      memShellBtn.setOnAction((event) -> {
         ...
         if (!scriptType.equals("jsp")) {
            ...
            ComboBox typeCombo = new ComboBox();
            typeCombo.setItems(FXCollections.observableArrayList("AgentNoFile", "Agent"));
   ...
   private void fillShellRows(JSONArray jsonArray) {
      ...
      for(int i = 0; i < jsonArray.length(); i++) {
         ...
         try {
            ...
            row.add(0, new SimpleStringProperty(id + ""));
            row.add(1, new SimpleStringProperty(url));
            row.add(2, new SimpleStringProperty(ip));
            row.add(3, new SimpleStringProperty(type));
            row.add(4, new SimpleStringProperty(os));
            row.add(5, new SimpleStringProperty(comment));
            row.add(6, new SimpleStringProperty(addTime));
            row.add(7, new SimpleStringProperty(status + ""));
            data.add(row);
         } catch (Exception var16) {
            var16.printStackTrace();
         }
