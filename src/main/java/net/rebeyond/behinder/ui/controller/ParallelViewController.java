...
public class ParallelViewController {
   ...
   public void init(IShellService shellService, PluginViewController pluginViewController, PluginService pluginService, List<Thread> workList, List<Task> taskList, Label statusLabel, ShellManager shellManager, Map<String, String> basicInfoMap) {
   ...
   private void initTunnelContextMenu() {
      ...
      cm.getItems().addAll(toggleBtn, delBtn, cloneBtn, reOpenBtn);
      ...
      clearBtn.setOnAction((event) -> {
         Optional<ButtonType> buttonType = Utils.showConfirmMessage("提示", "确认清空所有隧道？");
      ...
      this.tunnelViewContextMenu.getItems().addAll(refreshBtn, closeAllBtn, openAllBtn, clearBtn);
   ...
   private void scanAliveHosts(String ips, boolean needClear) {
      ...
      paramObj.put("ipList", (Object)ips);
      paramObj.put("threadSize", (Object)"20");
   ...
   private void scanService(final String hostList, final String portList, final String threadSize) {
      ...
      paramObj.put("hostList", (Object)hostList);
      paramObj.put("portList", (Object)portList);
      paramObj.put("probeFilePath", (Object)this.probeFilePath);
      paramObj.put("threadSize", (Object)threadSize);
   ...
   private void fillServiceGroup(List<Service> serviceList) {
      Map<String, Integer> servicePortGroup = new HashMap();
      Map<String, Integer> serviceNameGroup = new HashMap();
   ...
   private void loadServices(Host host) throws Exception {
      List<Service> serviceList = this.serviceDao.findServiceByHostId(host.getId());
   ...
   private void fillServiceGroup(Map<String, Integer> servicePortGroup, Map<String, Integer> serviceNameGroup) {
   ...
   private void scrollToService(Service service) {
      List<Node> serviceBoxList = this.serviceDetailFlowPane.getChildren();
   ...
   private void loadHosts() throws Exception {
      this.hostFlowPane.getChildren().clear();
      this.loadLocalHost();
      List<Host> hostList = this.hostDao.findHostByShellId(this.shellID);
   ...
   private void loadHosts(List<Host> hostList) throws Exception {
      this.hostFlowPane.getChildren().clear();
      Iterator var2 = hostList.iterator();
   ...
   private void loadServices() throws Exception {
      this.serviceFlowPane.getChildren().clear();
      List<Service> serviceList = this.serviceDao.findServiceByShellId(this.shellID);
   ...
   private void loadServices(List<Service> serviceList) throws Exception {
      this.serviceFlowPane.getChildren().clear();
      Iterator var2 = serviceList.iterator();
   ...
   private JSONArray loadPluginList() {
      ...
      pluginScan.put("name", (Object)"扫描类");
      pluginScan.put("leaf", (Object)"false");
      JSONArray pluginScanArr = new JSONArray();
      pluginScan.put("children", (Object)pluginScanArr);
      JSONObject pluginExploit = new JSONObject();
      pluginExploit.put("name", (Object)"利用类");
      pluginExploit.put("leaf", (Object)"false");
      JSONArray pluginExploitArr = new JSONArray();
      pluginExploit.put("children", (Object)pluginExploitArr);
      JSONObject pluginTool = new JSONObject();
      pluginTool.put("name", (Object)"工具类");
      pluginTool.put("leaf", (Object)"false");
      JSONArray pluginToolArr = new JSONArray();
      pluginTool.put("children", (Object)pluginToolArr);
      JSONObject pluginOther = new JSONObject();
      pluginOther.put("name", (Object)"其他类");
      pluginOther.put("leaf", (Object)"false");
      JSONArray pluginOtherArr = new JSONArray();
      pluginOther.put("children", (Object)pluginOtherArr);
      pluginArr.put(Constants.PLUGIN_TYPE_SCAN, (Object)pluginScan);
      pluginArr.put(Constants.PLUGIN_TYPE_EXPLOIT, (Object)pluginExploit);
      pluginArr.put(Constants.PLUGIN_TYPE_TOOL, (Object)pluginTool);
      pluginArr.put(Constants.PLUGIN_TYPE_OTHER, (Object)pluginOther);

      try {
         List<Plugin> pluginList = this.pluginDao.findAllPlugins();
   ...
   private void initParaView() throws Exception {
      ...
      new Menu("GetShell");
      bShellMenu.getItems().addAll(manualAddBtn);
      cm.getItems().addAll(openBtn, scanServiceBtn, bShellMenu);
      ...
      commentBtn.setOnAction((event) -> {
         ...
         Alert alert = Utils.getAlert(Alert.AlertType.INFORMATION);
      ...
      this.fastServiceScanBtn.setOnAction((event) -> {
         ...
         Optional<ButtonType> result = Utils.showConfirmMessage("提示", "即将对下列IP列表进行全端口扫描：\n" + this.getLocalIpRange(true));
   ...
   private void initServiceDetailContextMenu() {
      ...
      portMapMenu.getItems().addAll(localPortMapBtn, remoteortMapBtn);
   ...
   private void initMainContextMenu() {
      ...
      scanHostBtn.setOnAction((event) -> {
         if (this.hostFlowPane.getChildren().size() > 1) {
            Optional<ButtonType> result = Utils.showConfirmMessage("确认", "该操作会清空现有主机列表并重新扫描，是否继续？");
            if (!result.isPresent()) {
               return;
            }
      ...
      cm.getItems().addAll(scanHostBtn, scanServiceBtn, refreshBtn);
   ...
   private void initBShellBoxContextMenu() {
      ...
      cm.getItems().addAll(openConsoleBtn, openGUIBtn, delBShellBtn);
                     ...
                     for(this.waitReverse = true; this.waitReverse; Thread.sleep(10000L)) {
                        ...
                        if (status.equals("success")) {
                           ...
                           for(int i = 0; i < reversBShellArr.length(); ++i) {
                              ...
                              bShellObj.put("type", (Object)"native");
                              bShellObj.put("listenPort", (Object)this.listenPort);
      ...
      refreshBtn.setOnAction((event) -> {
         ...
         try {
            ...
            for(int i = 0; i < bShellArr.length(); ++i) {
               ...
               bodyObj.put("type", (Object)"native");
               bodyObj.put("target", (Object)target);
      ...
      cm.getItems().addAll(addBtn, startReverseBtn, refreshBtn);
   ...
   private void addTunnelBox(Tunnel tunnel) throws Exception {
      ...
      box.getChildren().addAll(targetIpLabel, targetPortLabel, remoteIpLabel, remotePortLabel, statusLabel);
   ...
   private void addBShellBox(BShell bShell) throws Exception {
      ...
      box.getChildren().addAll(portLabel, serviceLabel, versionLabel);
      box.setStyle("-fx-border-width: 1; -fx-border-insets: 5; -fx-border-style: dashed; -fx-border-color: green;");
      box.setPadding(new Insets(5.0));
      box.setPrefWidth(300.0);
   ...
   private void addBShellReverseTemplate(String listenPort) {
      ...
      bannerLabel.setStyle("-fx-text-fill:#00ff00");
      box.getChildren().addAll(portLabel, serviceLabel, versionLabel, bannerLabel);
      box.setStyle("-fx-border-width: 1; -fx-border-insets: 5; -fx-border-style: dashed; -fx-border-color: green;");
      box.setPadding(new Insets(5.0));
      box.setPrefWidth(300.0);
      box.setSpacing(10.0);
      ...
      contextMenu.getItems().addAll(stopListen, reListen);
      box.setOnContextMenuRequested((event) -> {
   ...
   private void loadBShellList() {
      ...
      try {
         List<BShell> bShellList = this.bShellDao.findBShellByShellId(this.shellID);
   ...
   private void initHostDetail() throws Exception {
      ...
      box.getChildren().addAll(hostItem, hostIP, hostComment, hostOS);
      box.setStyle("-fx-border-width: 1; -fx-border-insets: 5; -fx-border-style: dashed; -fx-border-color: green;");
      box.setPadding(new Insets(5.0));
   ...
   private void addServiceDetailBox(Service service, FlowPane flowPane, boolean isShowIp) {
      ...
      if (isShowIp) {
         box.getChildren().addAll(portLabel, ipLabel, serviceLabel, versionLabel, bannerLabel);
      } else {
         box.getChildren().addAll(portLabel, serviceLabel, versionLabel, bannerLabel);
      }
   ...
   private void loadTunnelFlowView(boolean initDead) {
      try {
         List<Tunnel> tunnelList = this.tunnelDao.findTunnelByShellId(this.shellID);
   ...
   private JSONObject getChildShellEntity(BShell bShell) throws Exception {
      ...
      childShellEntity.put("url", (Object)url);
      childShellEntity.put("type", (Object)scriptType);
      childShellEntity.put("password", this.shellEntity.get("password"));
      childShellEntity.put("bShell", (Object)bShell);
   ...
   private ShellService getChildShellService(BShell bShell) throws Exception {
      ...
      childObj.put("childShellEntity", childShellEntity);
      childObj.put("bShell", bShell);
      List<Map<String, Object>> childList = this.getChildList(childShellEntity, bShell);
   ...
   private void showAddBShellPane(String IPAddress) {
      Alert inputDialog = Utils.getAlert(Alert.AlertType.NONE);
      ...
      proxyGridPane.getColumnConstraints().addAll(firstColConfig, secondColConfig);
      proxyGridPane.setPadding(new Insets(20.0, 20.0, 0.0, 10.0));
      Label typeLabel = new Label("类型：");
      ComboBox typeCombo = new ComboBox();
      typeCombo.setItems(FXCollections.observableArrayList("TCP", "HTTP"));
      typeCombo.getSelectionModel().select(0);
      ComboBox scriptTypeCombo = new ComboBox();
      scriptTypeCombo.setItems(FXCollections.observableArrayList("jsp", "php", "aspx", "asp"));
      ...
      saveBtn.setOnAction((e) -> {
         ...
         if (!Utils.checkIP(targetIp)) {
            ...
            if (bShellType == Constants.BSHELL_TYPE_TCP) {
               bShellBody.put("target", (Object)(target + ":" + portText.getText()));
               bShellBody.put("type", (Object)"native");
            } else if (bShellType == Constants.BSHELL_TYPE_HTTP) {
               bShellBody.put("target", (Object)target);
   ...
   private void showCustomServiceScan() {
      ...
      secondColConfig.setPercentWidth(80.0);
      secondColConfig.setFillWidth(true);
      customServiceGridPane.getColumnConstraints().addAll(firstColConfig, secondColConfig);
