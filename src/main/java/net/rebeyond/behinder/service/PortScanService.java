...
public class PortScanService implements Runnable {
   ...
   public PortScanService(BlockingQueue blockingQueue, Map<String, Object> taskResult, IShellService shellService) {
   ...
   public void execute(IShellService shellService, Map<String, Object> taskResult) throws Exception {
      this.dispatch();
   ...
   private Map<String, String> checkProbe(String type, String target, Map<String, Object> probe) throws Exception {
      ...
      if (responseObj.getString("status").equals("success")) {
         ...
         for(int i = 0; i < matchArr.size(); ++i) {
            Map<String, Object> matchObj = (Map)matchArr.get(i);
   ...
   private List buildProbeIndexList(List probeArr, int port) {
      ...
      for(int i = 0; i < probeArr.size(); ++i) {
         Map<String, Object> probeObj = (Map)probeArr.get(i);
   ...
   private String fillVersion(Matcher matcher, Map<String, String> versionObj) {
   ...
   public void run() {
      while(!this.taskResult.get("running").equals("false")) {
         ...
         List<Integer> probeIndexList = this.buildProbeIndexList(probeArr, port);
         String finalBanner = "";
         ...
         while(var10.hasNext()) {
            ...
            Map<String, Object> probeObj = (Map)probeArr.get(index);
            String type = probeObj.get("type").toString();
            if ((!type.equals("TCP") || !skipTCP) && (!type.equals("UDP") || !skipUDP)) {
               try {
                  Map<String, String> result = this.checkProbe(type, target, probeObj);
