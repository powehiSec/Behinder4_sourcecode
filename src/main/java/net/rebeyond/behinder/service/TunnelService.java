...
public class TunnelService {
   private Map<String, ServerSocketChannel> localPortMapServerSocket = new HashMap();
   private Map<String, ServerSocketChannel> localSocksProxyServerSocket = new HashMap();
   ...
   private Map<String, List<Thread>> localPortMapThreadMap = new HashMap();
   private Map<String, List<Thread>> localSocksProxyThreadMap = new HashMap();
   private List<Thread> workList;
   private ITunnelCallBack callBack;
   private Map<String, List<SocketChannel>> localPortMapSocketList = new HashMap();
   private Map<String, List<SocketChannel>> localSocksProxySocketList = new HashMap();

   public TunnelService(IShellService shellService, List<Thread> workList, ITunnelCallBack callBack) {
   ...
   public void createLocalSocksProxy(String localIp, String localPort) {
      List<Thread> serviceThreadList = (List)this.localSocksProxyThreadMap.get(localPort);
      if (serviceThreadList == null) {
         serviceThreadList = new ArrayList();
         this.localSocksProxyThreadMap.put(localPort, serviceThreadList);
      }

      List<Thread> finalServiceThreadList = serviceThreadList;
      List<Thread> finalServiceThreadList1 = serviceThreadList;
      Runnable runner = () -> {
         try {
            ...
            while(true) {
               ...
               List<Thread> singleTaskList = new ArrayList();
               this.callBack.onInfo("收到客户端连接请求.");
               List<SocketChannel> socketChannelList = (List)this.localSocksProxySocketList.get(localPort);
               ...
               worker.start();
               finalServiceThreadList1.add(worker);
            }
   ...
   public void createLocalPortMap(String localIp, String localPort, String targetIp, String targetPort) {
      ...
      Runnable runner = () -> {
         try {
            ...
            while(true) {
               SocketChannel socketChannel = serverSocketChannel.accept();
               List<SocketChannel> socketChannelList = (List)this.localPortMapSocketList.get(localPort);
               if (socketChannelList == null) {
                  socketChannelList = new ArrayList();
   ...
   private void stopWorkers(List<Thread> workList) {
   ...
   private void stopLocalSocksProxyAllWorkers(String localPort) {
      ...
      List<Thread> threadList = (List)this.localSocksProxyThreadMap.get(localPort);
   ...
   private void stopLocalPortMapAllWorkers(String localPort) {
      ...
      List<Thread> threadList = (List)this.localPortMapThreadMap.get(localPort);
      Iterator var3 = threadList.iterator();
   ...
   private boolean parseSocks5(Socket socket, String socketHash) throws Exception {
      ...
      if (atyp == 1) {
         ...
         for(int var17 = 0; var17 < temp; ++var17) {
            temp = Integer.parseInt(var23[var17]);
            host = host + temp + ".";
         }
